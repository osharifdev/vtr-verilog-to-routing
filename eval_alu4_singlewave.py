...
def main():
    report_lines = [
        "# Single-Wave Budgeted Portfolio Evaluation",
    ...
    for cores in CORES_LIST:
        ...
        for seed in SEEDS:
            out_dir = f"eval_singlewave_{cores}_seed{seed}"
            cmd = [
            ...
            baseline_lat = None
            best_cpd = None
            budget_lat = 0.0 # max over all runs
        ...
        # Geomean of % differences
        # To avoid log(<=0), we compute the geomean of speedup ratios then convert
    ...
    report_content = "\n".join(report_lines)
    with open("singlewave_alu4_report.md", "w") as f:
